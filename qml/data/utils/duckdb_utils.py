...
def load_table_df(connection,table_name,column_names=None,filter=None,load_sql=None):
    if column_names is None:
        column_names = "*"
    if load_sql is None:
        if filter is None:
            return connection.execute(f"select {column_names} from {table_name}").df()
        else:
            return connection.execute(f"select {column_names} from {table_name} where {filter}").df()
    else:
        return connection.execute(load_sql).df()
