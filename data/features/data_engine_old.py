...
class LabelCreator(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        ...
        df = df.sort_index()
        df = df[~df.index.duplicated(keep='first')]
        df[self.label_name] = df.shift(self.shift, freq=self.freq)[
            self.shift_column].subtract(df[self.shift_column]).apply(self.label_generator)
...
class TechnicalIndicator(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        self.all_methods = [
            m for m in self.all_methods for mt in self.method_type if mt in m]
        return self    # Nothing to do in fit in this scenario
...
class CreateTechnicalIndicatorUsingPandasTA(BaseEstimator, TransformerMixin):
    def __init__(self, exclude=["pvo", "vwap", "vwma", "ad", "adosc", "aobv", "cmf", "efi", "eom", "kvo", "mfi", "nvi", "obv", "pvi", "pvol", "pvr", "pvt"], verbose=True):
    ...
    def transform(self, df):
        ...
        if self.verbose:
            print_log(
                f"Shape of dataframe before CreateTechnicalIndicatorUsingPandasTA is {df.shape}")
        df.ta.strategy(exclude=self.exclude, verbose=self.verbose, timed=True)
        if self.verbose:
            print_log(
                f"Shape of dataframe after CreateTechnicalIndicatorUsingPandasTA is {df.shape}")
        return df
...
class CreateTechnicalIndicatorUsingTA(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        ...
        if self.verbose:
            print_log(
                f"Shape of dataframe before CreateTechnicalIndicatorUsingTA is {df.shape}")
        ...
        if self.verbose:
            print_log(
                f"Shape of dataframe after CreateTechnicalIndicatorUsingTA is {df.shape}")
        return df
...
class NormalizeDataset(BaseEstimator, TransformerMixin):
    ...
    def fit(self, df, y=None):
        if len(self.columns) == 0:
            self.columns = [m for m in df.columns.tolist()
                            for mt in self.column_pattern if mt in m]
    ...
    def transform(self, df):
        ...
        if self.impute_values:
            ...
            if self.impute_type == 'mean_median_imputer':
                imputer = MeanMedianImputer(
                    imputation_method='median', variables=self.columns)
                ...
                if isinstance(self.arbitrary_impute_variable, dict):
                    imputer = ArbitraryNumberImputer(
                        imputer_dict=self.arbitrary_impute_variable)

                else:
                    imputer = ArbitraryNumberImputer(
                        variables=self.columns, arbitrary_number=self.arbitrary_number)
        ...
        df = df.sort_index()
        print_log(
            f"Shape of dataframe after NormalizeDataset is {df.shape} : {'.'.join(info_list)}")
        return df
...
class LastTicksGreaterValuesCount(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        ...
        for col in self.columns:
            print_log(
                f"LastTicksGreaterValuesCount : {col} : f'last_tick_{col}_{self.last_ticks}'")
            ...
            if self.create_new_col:
                ...
                df[f'last_tick_{col}_{self.last_ticks}'] = (
                    arr[:, :-1] > arr[:, [-1]]).sum(axis=1)
            else:
                df[col] = (arr[:, :-1] > arr[:, [-1]]).sum(axis=1)
        print_log(
            f"Shape of dataframe after LastTicksGreaterValuesCount is {df.shape}")
        return df
        ...
        for col in self.columns:
            for breach_type in self.breach_type:
                ...
                if self.create_new_col:
                    ...
                    col_name = col
                print_log(
                    f"PriceLastTickBreachCount : {breach_type} : {col} : {col_name}")
                if breach_type == 'morethan':
                    df[col_name] = df[col].rolling(self.last_ticks, min_periods=1).apply(
                        lambda x: (x[-1] > x[:-1]).sum()).fillna(0)
                elif breach_type == 'lessthan':
                    df[col_name] = df[col].rolling(self.last_ticks, min_periods=1).apply(
                        lambda x: (x[-1] < x[:-1]).sum()).fillna(0)
                elif breach_type == 'mean':
                    df[col_name] = df[col].rolling(self.last_ticks, min_periods=1).apply(
                        lambda x: (x > x[:].mean()).sum()).fillna(0).astype(int)
                elif breach_type == 'min':
                    df[col_name] = df[col].rolling(self.last_ticks, min_periods=1).apply(
                        lambda x: (x > x[:].min()).sum()).fillna(0).astype(int)
                elif breach_type == 'max':
                    df[col_name] = df[col].rolling(self.last_ticks, min_periods=1).apply(
                        lambda x: (x > x[:].max()).sum()).fillna(0).astype(int)
                elif breach_type == 'median':
                    df[col_name] = df[col].rolling(self.last_ticks, min_periods=1).apply(
                        lambda x: (x > x[:].median()).sum()).fillna(0).astype(int)
                elif breach_type == '10thquantile':
                    df[col_name] = df[col].rolling(self.last_ticks, min_periods=1).apply(
                        lambda x: (x > x[:].quantile(0.1)).sum()).fillna(0).astype(int)
                elif breach_type == '25thquantile':
                    df[col_name] = df[col].rolling(self.last_ticks, min_periods=1).apply(
                        lambda x: (x > x[:].quantile(0.25)).sum()).fillna(0).astype(int)
                elif breach_type == '75thquantile':
                    df[col_name] = df[col].rolling(self.last_ticks, min_periods=1).apply(
                        lambda x: (x > x[:].quantile(0.75)).sum()).fillna(0).astype(int)
                elif breach_type == '95thquantile':
                    df[col_name] = df[col].rolling(self.last_ticks, min_periods=1).apply(
                        lambda x: (x > x[:].quantile(0.95)).sum()).fillna(0).astype(int)
                else:
                    df[col_name] = (df[col].rolling(self.last_ticks, min_periods=1)
                                    .apply(lambda x: (x[-1] > x[:-1]).mean())
                                    .astype(int))
        print_log(
            f"Shape of dataframe after PriceLastTickBreachCount is {df.shape}")
        return df
...
class ValueLastTickBreachCount(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        ...
        for col in self.columns:
            for breach_type in self.breach_type:
                ...
                if self.create_new_col:
                    ...
                    col_name = col
                print_log(
                    f"ValueLastTickBreachCount : {breach_type} : {col} : {col_name}")
                if breach_type == 'morethan':
                    df[col_name] = df[col].rolling(self.last_ticks, min_periods=1).apply(
                        lambda x: (x[-1] > x[:-1]).sum()).fillna(0)
                elif breach_type == 'lessthan':
        ...
        if self.self.verbose:
            print_log(
                f"Shape of dataframe after ValueLastTickBreachCount is {df.shape}")
        return df
...
class PriceLastTickBreachCount(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        ...
        for breach_type in self.breach_type:
            print_log(
                f"PriceLastTickBreachCount : {breach_type} : {self.last_ticks}")
            if breach_type == 'morethan':
                tmpdf = df[self.columns].rolling(self.last_ticks, min_periods=1).apply(
                    lambda x: sum(x[-1] > np.array(x[:-1]))).fillna(0)
            elif breach_type == 'lessthan':
                tmpdf = df[self.columns].rolling(self.last_ticks, min_periods=1).apply(
                    lambda x: sum(x[-1] < np.array(x[:-1]))).fillna(0)
            elif breach_type == 'mean':
                tmpdf = df[self.columns].rolling(self.last_ticks, min_periods=1).apply(
                    lambda x: sum(x > np.mean(np.array(x)))).fillna(0).astype(int)
            elif breach_type == 'min':
                tmpdf = df[self.columns].rolling(self.last_ticks, min_periods=1).apply(
                    lambda x: sum(x > np.min(np.array(x)))).fillna(0).astype(int)
            elif breach_type == 'max':
                tmpdf = df[self.columns].rolling(self.last_ticks, min_periods=1).apply(
                    lambda x: sum(x > np.max(np.array(x)))).fillna(0).astype(int)
            elif breach_type == 'median':
                tmpdf = df[self.columns].rolling(self.last_ticks, min_periods=1).apply(
                    lambda x: sum(x > np.median(np.array(x)))).fillna(0).astype(int)
            elif breach_type == '10thquantile':
                tmpdf = df[self.columns].rolling(self.last_ticks, min_periods=1).apply(
                    lambda x: sum(x > np.quantile(np.array(x), 0.1))).fillna(0).astype(int)
            elif breach_type == '25thquantile':
                tmpdf = df[self.columns].rolling(self.last_ticks, min_periods=1).apply(
                    lambda x: sum(x > np.quantile(np.array(x), 0.25))).fillna(0).astype(int)
            elif breach_type == '75thquantile':
                tmpdf = df[self.columns].rolling(self.last_ticks, min_periods=1).apply(
                    lambda x: sum(x > np.quantile(np.array(x), 0.75))).fillna(0).astype(int)
            elif breach_type == '95thquantile':
                tmpdf = df[self.columns].rolling(self.last_ticks, min_periods=1).apply(
                    lambda x: sum(x > np.quantile(np.array(x), 0.95))).fillna(0).astype(int)
            else:
                tmpdf = df[self.columns].rolling(self.last_ticks, min_periods=1).apply(
                    lambda x: sum(x[-1] > np.array(x[:-1]))).fillna(0)
            col_names = [
                f"{col}_{self.last_ticks}_{'_'.join(self.breach_type)}_last_tick_" for col in self.columns]
            tmpdf.columns = col_names
            df = pd.merge(df, tmpdf, left_index=True,
...
class RollingValues(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        ...
        col_names = [
            f"{shftcol}_{'_'.join(self.last_ticks)}_{'_'.join(self.aggs)}_rolling_values" for shftcol in self.columns]
        tmpdf.columns = col_names
    ...
    def transform(self, df):
        ...
        for r1, r2 in self.hour_range:
            for rt in self.range_type:
                print_log(
                    f"PriceDayRangeHourWise : {self.first_col} : {self.second_col} : {r1} : {r2} : {rt}")
                if rt == 'price_range':
                    s1 = df[self.first_col].between_time(r1, r2).groupby(pd.Grouper(freq='d')).max(
                    ) - df[self.second_col].between_time(r1, r2).groupby(pd.Grouper(freq='d')).min()
                elif rt == 'price_deviation_max_first_col':
                    s1 = df[self.first_col].between_time(r1, r2).groupby(pd.Grouper(freq='d')).mean(
                    ) - df[self.first_col].between_time(r1, r2).groupby(pd.Grouper(freq='d')).max()
                elif rt == 'price_deviation_min_first_col':
                    s1 = df[self.first_col].between_time(r1, r2).groupby(pd.Grouper(freq='d')).mean(
                    ) - df[self.first_col].between_time(r1, r2).groupby(pd.Grouper(freq='d')).min()
                elif rt == 'price_deviation_max_second_col':
                    s1 = df[self.second_col].between_time(r1, r2).groupby(pd.Grouper(freq='d')).mean(
                    ) - df[self.second_col].between_time(r1, r2).groupby(pd.Grouper(freq='d')).max()
            ...
            s1.name = col_name
            df = pd.merge(df, s1, how='outer',
                          left_index=True, right_index=True)
            df[col_name] = df[col_name].fillna(method='ffill')
        print_log(
            f"Shape of dataframe after PriceDayRangeHourWise is {df.shape}")
        return df
...
class PriceVelocityv2(BaseEstimator, TransformerMixin):
    ...
    def fit(self, df, y=None):
        if len(self.shift_column) == 0:
            self.shift_column = [m for m in df.columns.tolist()
                                 for mt in self.shift_column_pattern if mt in m]
    ...
    def transform(self, df):
        ...
        for shftcol in self.shift_column:
            ...
            if self.freq is not None:
                ...
                df = pd.merge(df, a, left_index=True,
                              right_index=True, how='left')
...
class PriceVelocity(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        ...
        df = df.sort_index()
        if self.freq is not None:
            tmpdf = df[self.shift_column].subtract(
                df.shift(self.shift, freq=self.freq)[self.shift_column])
            col_names = [
                f'{shftcol}_{self.freq}_{self.shift}_price_velocity' for shftcol in self.shift_column]
            tmpdf.columns = col_names
        else:
            tmpdf = df[self.shift_column].subtract(
                df.shift(self.shift)[self.shift_column])
            col_names = [
                f'{shftcol}_{self.shift}_price_velocity' for shftcol in self.shift_column]
            tmpdf.columns = col_names
        df = pd.merge(df, tmpdf, left_index=True, right_index=True, how='left')
        if self.verbose:
            print_log(f"Shape of dataframe after PriceVelocity is {df.shape}")
    ...
    def transform(self, df):
        ...
        for shftcol in self.shift_column:
            ...
            if self.freq is not None:
                ...
                df[self.col_name] = df[shftcol] - \
                    df.shift(self.shift, freq=self.freq)[shftcol]
                df[self.col_name] = df[self.col_name].round(3)
            else:
                self.col_name = f'price_velocity_{shftcol}_{self.shift}'
...
class PricePerIncrement(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        ...
        if self.freq is not None:
            ...
            col_names = [
                f'{shftcol}_{self.freq}_{self.shift}_price_per_velocity' for shftcol in self.shift_column]
            tmpdf.columns = col_names
        else:
            tmpdf = df[self.shift_column].subtract(
                df.shift(self.shift)[self.shift_column])
            col_names = [
                f'{shftcol}_{self.shift}_price_per_velocity' for shftcol in self.shift_column]
            tmpdf.columns = col_names
        tmpdf = tmpdf/self.shift
        df = pd.merge(df, tmpdf, left_index=True, right_index=True, how='left')
        if self.verbose:
            print_log(
                f"Shape of dataframe after PricePerVelocity is {df.shape}")
        return df
...
class Zscoring(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        ...
        for col in self.columns:
            merge_dict.update(
                {f'Zscore_{col}_{self.window}': self.zscore(df[col], self.window)})
...
class PercentageChange(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        ...
        if self.verbose:
            print_log(
                f"Shape of dataframe before PercentageChange is {df.shape}")
        merge_dict = {}
        for col in self.columns:
            merge_dict.update({f'PerChg_{col}_{self.periods}_{self.freq}': df[col].pct_change(
                periods=self.periods, fill_method=self.fill_method, limit=self.limit, freq=self.freq)})
        ...
        if self.verbose:
            print_log(
                f"Shape of dataframe after PercentageChange is {df.shape}")
        return df
...
class PercentageChange_Multiplier(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        ...
        for col in self.columns:
            merge_dict.update({f'PerChg_{col}_{self.periods}_{self.freq}': df[col].pct_change(
                periods=self.periods, fill_method=self.fill_method, limit=self.limit, freq=self.freq)*self.multiplier})
            # df[f'PerChg_{col}_{self.periods}_{self.freq}'] =df[col].pct_change(periods=self.periods,fill_method=self.fill_method,limit = self.limit,freq=self.freq)
            print_log(f"PerChg_{col}_{self.periods}_{self.freq} completed")
        df = pd.concat([df, pd.concat(merge_dict, axis=1)], axis=1)
        if self.verbose:
...
class WeightedExponentialAverage(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        ...
        if self.verbose:
            print_log(
                f"Shape of dataframe before WeightedExponentialAverage is {df.shape}")
        merge_dict = {}
        for col in self.columns:
            merge_dict.update({f'WEA_{col}_{self.span}': df[col].ewm(com=self.com, span=self.span, halflife=self.halflife, alpha=self.alpha,
                              min_periods=self.min_periods, adjust=self.adjust, ignore_na=self.ignore_na, axis=self.axis, times=self.times).mean()})
            print_log(f"WEA_{col}_{self.span} completed")
        df = pd.concat([df, pd.concat(merge_dict, axis=1)], axis=1)
        if self.verbose:
            print_log(
                f"Shape of dataframe after WeightedExponentialAverage is {df.shape}")
        return df
...
class PercentileTransform(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        ...
        if self.verbose:
            print_log(
                f"Shape of dataframe before PercentileTransform is {df.shape}")
        ...
        merge_dict = {}
        for col in self.columns:
            merge_dict.update({f'PCTL_{col}_{self.window}_{self.min_periods}': df[col].rolling(
                self.window, min_periods=self.min_periods).apply(lambda x: np.array(x)[-1] - np.quantile(np.array(x), q=self.quantile))})
            print_log(f"PCTL_{col}_{self.window}_{self.min_periods} completed")
        df = pd.concat([df, pd.concat(merge_dict, axis=1)], axis=1)
        if self.verbose:
            print_log(
                f"Shape of dataframe after PercentileTransform is {df.shape}")
        return df
...
class RollingRank(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        ...
        merge_dict = {}
        for col in self.columns:
            merge_dict.update({f'RRNK_{col}_{self.window}_{self.min_periods}': df[col].rolling(
                window=self.window, min_periods=self.min_periods).apply(self.rank)})
...
class BinningTransform(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        ...
        if self.verbose:
            print_log(
                f"Shape of dataframe before BinningTransform is {df.shape}")
        if self.get_current_row_bin:
            def bin_roll_fxn(x): return pd.qcut(
                np.array(x), labels=False, q=self.n_bins, duplicates='drop')[-1]
        else:
            def bin_roll_fxn(x): return pd.qcut(
                np.array(x), labels=False, q=self.n_bins, duplicates='drop')[0]
        merge_dict = {}
        for col in self.columns:
            merge_dict.update({f'BINT_{col}_{self.window}_{self.min_period}_{self.n_bins}': df[col].rolling(
                window=self.window, min_periods=self.min_period).apply(bin_roll_fxn)})
            print_log(
                f"BINT_{col}_{self.window}_{self.min_period}_{self.n_bins} completed")
        df = pd.concat([df, pd.concat(merge_dict, axis=1)], axis=1)
        if self.verbose:
            print_log(
                f"Shape of dataframe after BinningTransform is {df.shape}")
        return df
...
class PositiveNegativeTrends(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        ...
        if self.verbose:
            print_log(
                f"Shape of dataframe before PositiveNegativeTrends is {df.shape}")
        merge_dict = {}
        for col in self.columns:
            merge_dict.update({f'PNT_{col}_{self.window}_{self.min_periods}_DIFF': df[col].pct_change(
            ).apply(np.sign).rolling(self.window, min_periods=self.min_periods).apply(np.sum)})
            print_log(
                f"PNT_{col}_{self.window}_{self.min_periods}_DIFF completed")
        df = pd.concat([df, pd.concat(merge_dict, axis=1)], axis=1)
        if self.verbose:
            print_log(
                f"Shape of dataframe after PositiveNegativeTrends is {df.shape}")
        return df
...
class Rolling_Stats(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        ...
        merge_dict = {}
        for col in self.columns:
            merge_dict.update({f'RR_{col}_{self.window}_{self.min_periods}_DIFF': df[col].rolling(
                self.window, min_periods=self.min_periods).apply(lambda x: np.array(x)[-1]-np.array(x)[0])})
            print_log(
                f"RR_{col}_{self.window}_{self.min_periods}_DIFF completed")
            merge_dict.update({f'RR_{col}_{self.window}_{self.min_periods}_MAXDIFF': df[col].rolling(
                self.window, min_periods=self.min_periods).apply(lambda x: np.array(x)[-1]-max(np.array(x)))})
            print_log(
                f"RR_{col}_{self.window}_{self.min_periods}_MAXDIFF completed")
            merge_dict.update({f'RR_{col}_{self.window}_{self.min_periods}_MINDIFF': df[col].rolling(
                self.window, min_periods=self.min_periods).apply(lambda x: np.array(x)[-1]-min(np.array(x)))})
            print_log(
                f"RR_{col}_{self.window}_{self.min_periods}_MINDIFF completed")
            merge_dict.update({f'RR_{col}_{self.window}_{self.min_periods}_MEANDIFF': df[col].rolling(
                self.window, min_periods=self.min_periods).apply(lambda x: np.array(x)[-1]-mean(np.array(x)))})
            print_log(
                f"RR_{col}_{self.window}_{self.min_periods}_MEANDIFF completed")
            merge_dict.update({f'RR_{col}_{self.window}_{self.min_periods}_MAXMIN': df[col].rolling(
                self.window, min_periods=self.min_periods).apply(lambda x: max(np.array(x))-min(np.array(x)))})
            print_log(
                f"RR_{col}_{self.window}_{self.min_periods}_MAXMIN completed")
        df = pd.concat([df, pd.concat(merge_dict, axis=1)], axis=1)
        if self.verbose:
...
class Rolling_Stats_withLookBack(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        ...
        if self.verbose:
            print_log(
                f"Shape of dataframe before Rolling_Stats_withLookBack is {df.shape}")
        merge_dict = {}
        for col in self.columns:
            merge_dict.update({f'RRLB_{col}_{self.window}_{self.min_periods}_DIFF': df[col].rolling(
                self.window, min_periods=self.min_periods).apply(lookback_diff)})
            print_log(
                f"RRLB_{col}_{self.window}_{self.min_periods}_DIFF completed")
            merge_dict.update({f'RRLB_{col}_{self.window}_{self.min_periods}_MAXDIFF': df[col].rolling(
                self.window, min_periods=self.min_periods).apply(lookback_max)})
            print_log(
                f"RRLB_{col}_{self.window}_{self.min_periods}_MAXDIFF completed")
            merge_dict.update({f'RRLB_{col}_{self.window}_{self.min_periods}_MINDIFF': df[col].rolling(
                self.window, min_periods=self.min_periods).apply(lookback_min)})
            print_log(
                f"RRLB_{col}_{self.window}_{self.min_periods}_MINDIFF completed")
            merge_dict.update({f'RRLB_{col}_{self.window}_{self.min_periods}_MEANDIFF': df[col].rolling(
                self.window, min_periods=self.min_periods).apply(lookback_mean)})
            print_log(
                f"RRLB_{col}_{self.window}_{self.min_periods}_MEANDIFF completed")
            merge_dict.update({f'RRLB_{col}_{self.window}_{self.min_periods}_MAXMAX': df[col].rolling(
                self.window, min_periods=self.min_periods).apply(lookback_max_max)})
            print_log(
                f"RRLB_{col}_{self.window}_{self.min_periods}_MAXMAX completed")
            merge_dict.update({f'RRLB_{col}_{self.window}_{self.min_periods}_MINMIN': df[col].rolling(
                self.window, min_periods=self.min_periods).apply(lookback_min_min)})
            print_log(
                f"RRLB_{col}_{self.window}_{self.min_periods}_MINMIN completed")
        df = pd.concat([df, pd.concat(merge_dict, axis=1)], axis=1)
        if self.verbose:
            print_log(
                f"Shape of dataframe after Rolling_Stats_withLookBack is {df.shape}")
        return df
...
class Rolling_Stats_withLookBack_Compare(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        ...
        if self.verbose:
            print_log(
                f"Shape of dataframe before Rolling_Stats_withLookBack_Compare is {df.shape}")
        merge_dict = {}
        for col in self.columns:
            merge_dict.update({f'RRLBC_{col}_{self.window}_{self.min_periods}_DIFF': df[col].rolling(
                self.window, min_periods=self.min_periods).apply(lookback_diff)})
            print_log(
                f"RRLBC_{col}_{self.window}_{self.min_periods}_DIFF completed")
            merge_dict.update({f'RRLBC_{col}_{self.window}_{self.min_periods}_MAXDIFF': df[col].rolling(
                self.window, min_periods=self.min_periods).apply(lookback_max)})
            print_log(
                f"RRLBC_{col}_{self.window}_{self.min_periods}_MAXDIFF completed")
            merge_dict.update({f'RRLBC_{col}_{self.window}_{self.min_periods}_MINDIFF': df[col].rolling(
                self.window, min_periods=self.min_periods).apply(lookback_min)})
            print_log(
                f"RRLBC_{col}_{self.window}_{self.min_periods}_MINDIFF completed")
            merge_dict.update({f'RRLBC_{col}_{self.window}_{self.min_periods}_MEANDIFF': df[col].rolling(
                self.window, min_periods=self.min_periods).apply(lookback_mean)})
            print_log(
                f"RRLBC_{col}_{self.window}_{self.min_periods}_MEANDIFF completed")
            merge_dict.update({f'RRLBC_{col}_{self.window}_{self.min_periods}_MAXMIN': df[col].rolling(
                self.window, min_periods=self.min_periods).apply(lookback_max_min)})
            print_log(
                f"RRLBC_{col}_{self.window}_{self.min_periods}_MAXMIN completed")
            merge_dict.update({f'RRLBC_{col}_{self.window}_{self.min_periods}_MINMAX': df[col].rolling(
                self.window, min_periods=self.min_periods).apply(lookback_min_max)})
            print_log(
                f"RRLBC_{col}_{self.window}_{self.min_periods}_MINMAX completed")
        df = pd.concat([df, pd.concat(merge_dict, axis=1)], axis=1)
        if self.verbose:
            print_log(
                f"Shape of dataframe after Rolling_Stats_withLookBack_Compare is {df.shape}")
        return df
...
class PreviousDaysRange(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        ...
        if self.verbose:
            print_log(
                f"Shape of dataframe before PreviousDaysRange is {df.shape}")
        merge_dict = {}
        for col in self.columns:
            merge_dict.update({f'PVDR_{col}_{self.freq}': df[col].resample(self.resample).ffill().groupby(
                pd.Grouper(freq=self.freq)).apply(lambda x: np.array(x)[-1]-np.array(x)[0]).shift(self.shift)})
            merge_dict.update({f'PVDR_{col}_{self.freq}_MAX_MIN': df[col].resample(self.resample).ffill().groupby(
                pd.Grouper(freq=self.freq)).apply(lambda x: max(np.array(x))-min(np.array(x))).shift(self.shift)})
            print_log(f"PVDR_{col}_{self.freq}_{self.shift} completed")
        df = pd.merge_asof(df, pd.concat(merge_dict, axis=1),
                           left_index=True, right_index=True)
        if self.verbose:
            print_log(
                f"Shape of dataframe after PreviousDaysRange is {df.shape}")
        return df
...
class GapOpenMinuteChart(BaseEstimator, TransformerMixin):
    ...
    def transform(self, df):
        ...
        if self.verbose:
            print_log(
                f"Shape of dataframe before GapOpenMinuteChart is {df.shape}")
        merge_dict = {}
        for col in self.columns:
            tmp = df[col].resample('d').bfill().groupby(pd.Grouper(freq='d')).apply(lambda x: x[0]).subtract(
                df[col].resample('d').ffill().groupby(pd.Grouper(freq='d')).apply(lambda x: x[-1]).fillna(0))
        ...
        if self.verbose:
            print_log(
                f"Shape of dataframe after GapOpenMinuteChart is {df.shape}")
        return df
...
class ConvertUnstableCols(BaseEstimator, TransformerMixin):
    ...
    def fit(self, df, y=None):
        self.unstable_cols = [col for col in [c for c in df.columns.tolist(
        ) if df[c].dtypes != 'object'] if np.mean(df[col]) > self.tolerance]
        self.unstable_cols = [
            col for col in self.unstable_cols if col not in self.ohlc_columns]
        return self     # Nothing to do in fit in this scenario
    ...
    def transform(self, df):
        ...
        if self.verbose:
            print_log(
                f"Shape of dataframe before ConvertUnstableCols is {df.shape}")
        merge_dict = {}
        ...
        if self.verbose:
            print_log(
                f"Shape of dataframe after ConvertUnstableCols is {df.shape}")
        if self.transform_option == 'bint':
            bt_pipe = Pipeline([
                ('bt1', BinningTransform(columns=self.unstable_cols, window=15,
                 min_period=None, get_current_row_bin=True, n_bins=5, verbose=True)),
                ('bt2', BinningTransform(columns=self.unstable_cols, window=30,
                 min_period=None, get_current_row_bin=True, n_bins=5, verbose=True)),
                ('bt3', BinningTransform(columns=self.unstable_cols, window=45,
                 min_period=None, get_current_row_bin=True, n_bins=5, verbose=True)),
                ('bt4', BinningTransform(columns=self.unstable_cols, window=60,
                 min_period=None, get_current_row_bin=True, n_bins=5, verbose=True))
            ])
        elif self.transform_option == 'rank':
            bt_pipe = Pipeline([
                ('bt1', RollingRank(columns=self.unstable_cols,
                 window=15, min_periods=None, verbose=True)),
                ('bt2', RollingRank(columns=self.unstable_cols,
                 window=30, min_periods=None, verbose=True)),
                ('bt3', RollingRank(columns=self.unstable_cols,
                 window=45, min_periods=None, verbose=True)),
                ('bt4', RollingRank(columns=self.unstable_cols,
                 window=60, min_periods=None, verbose=True))
            ])
        else:
            bt_pipe = Pipeline([
                ('bt1', PercentageChange_Multiplier(columns=self.unstable_cols,
                 periods=15, fill_method='pad', limit=None, freq=None, verbose=True)),
                ('bt2', PercentageChange_Multiplier(columns=self.unstable_cols,
                 periods=30, fill_method='pad', limit=None, freq=None, verbose=True)),
                ('bt3', PercentageChange_Multiplier(columns=self.unstable_cols,
                 periods=45, fill_method='pad', limit=None, freq=None, verbose=True)),
                ('bt4', PercentageChange_Multiplier(columns=self.unstable_cols,
                 periods=60, fill_method='pad', limit=None, freq=None, verbose=True))
            ])
        df = bt_pipe.fit_transform(df)
        print_log(
            f"Shape of dataframe after applying transform in ConvertUnstableCols is {df.shape}")
        df = df.drop(self.unstable_cols, axis=1)
        print_log(
            f"Shape of dataframe after dropping unstable columns is {df.shape}")
        return df
